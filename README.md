...
- **联网搜索补充** — DuckDuckGo，safesearch='on'（引擎侧过滤，代码无需维护黑名单）
...
DuckDuckGo `safesearch='on'` 在引擎侧过滤违规内容，代码无需维护域名黑名单和关键词列表。
...
无模板。系统提示只要求：基于热榜资料 + 联网搜索自然回答，不要套任何模板、不要刻意凑字数。段落长短随内容而定。
...
- `safesearch='on'` 由引擎侧过滤违规内容，代码无需额外维护黑名单/关键词
