...
async def write_md_to_pdf(text: str, filename: str = "") -> str:
    ...
    try:
        ...
        # 使用 logger 而非 print，避免 weasyprint 在 Windows 控制台环境下
        # 关闭文件描述符后触发 "I/O operation on closed file"
        import logging
        logging.getLogger(__name__).info(f"Report written to {file_path}")
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"PDF 生成失败: {e}")
        return ""
