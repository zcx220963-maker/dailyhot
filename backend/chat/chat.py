...
class ChatAgentWithMemory:
    ...
    def _hot_items_prompt(self) -> str:
        ...
        return (
            ...
            "3. 结合报告中的已有分析，给出回答\n\n"
            "输出格式由用户问题决定，不要固定成某一种模板：\n"
            "- 用户要「口播稿」→ 约 750 字，口语化，开场白 + 3 段主体 + 结尾互动\n"
            "- 用户要「深度分析」→ 约 500 字，带小标题，背景→现状→影响→展望\n"
            "- 用户要「总结」→ 100 字以内，3 个 bullet\n"
            "- 用户问「怎么回事」「详细说说」「最新进展」等 → 自然段落回答，不要硬套模板"
        )
    ...
    async def chat(self, messages, websocket=None):
        ...
        try:
            ...
            system_parts = [
                ...
                "",
                "输出格式：",
                "- 格式由用户问题决定，不要固定成某一种模板：",
                "  - 用户要「口播稿」→ 约 750 字，口语化，开场白 + 3 段主体 + 结尾互动",
                "  - 用户要「深度分析」→ 约 500 字，带小标题，背景→现状→影响→展望",
                "  - 用户要「总结」→ 100 字以内，3 个 bullet",
                "  - 用户问「怎么回事」「详细说说」「最新进展」等 → 自然段落回答，不要硬套模板",
                "- 所有回答使用 Markdown 格式，段落清晰",
