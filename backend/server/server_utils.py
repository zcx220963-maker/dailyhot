...
async def _notify_feishu_ws(report: str, task: str):
    """Background helper for WebSocket path — send report to Feishu, swallow errors."""
    try:
        loop = asyncio.get_running_loop()
        ok = await loop.run_in_executor(None, send_report_to_feishu, report, task)
        if ok:
            logger.info(f"Feishu: WebSocket research '{task[:50]}...' pushed to Feishu successfully")
        else:
...
async def generate_report_files(report: str, filename: str) -> Dict[str, str]:
    """生成报告文件（PDF/DOCX/MD），单种格式失败不影响其他格式。"""
    file_paths: Dict[str, str] = {}
    # PDF — weasyprint 在 Windows 控制台环境下可能触发 I/O 错误，单独捕获
    try:
        file_paths["pdf"] = await write_md_to_pdf(report, filename)
    except Exception as e:
        logger.warning(f"PDF 生成失败（可忽略）: {e}")
        file_paths["pdf"] = ""
    # DOCX
    try:
        file_paths["docx"] = await write_md_to_word(report, filename)
    except Exception as e:
        logger.warning(f"DOCX 生成失败: {e}")
        file_paths["docx"] = ""
    # MD（保底）
    try:
        file_paths["md"] = await write_text_to_md(report, filename)
    except Exception as e:
        logger.warning(f"MD 生成失败: {e}")
        file_paths["md"] = ""
    return file_paths
...
def extract_command_data(json_data: Dict) -> tuple:
    return (
        ...
        json_data.get("hot_platforms", None),
    )
