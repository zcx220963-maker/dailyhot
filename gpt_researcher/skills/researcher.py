    ...
    async def conduct_research(self):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "starting_research",
                f"🔍 开始执行研究任务：'{self.researcher.query}'...",
                self.researcher.websocket,
            )
            # 将可能被 LLM 生成英文的 agent 名称映射为中文显示
            _agent_name_cn_map = {
                "News & Current Events Agent": "📰 新闻时事助手",
                "Finance Agent": "💰 金融分析师",
                "Business Analyst Agent": "📈 商业分析师",
                "Travel Agent": "🌍 旅行助手",
                "Academic Research Agent": "🎓 学术研究助手",
                "Computer Scientist Agent": "💻 计算机科学家",
                "Chemist Agent": "🧪 化学研究员",
                "Default Agent": "🤖 通用研究助手",
                "MCP Agent": "🔧 MCP 研究助手",
            }
            _agent_display = _agent_name_cn_map.get(
                self.researcher.agent.strip(), self.researcher.agent
            )
            await stream_output(
                "logs",
                "agent_generated",
                f"🤖 已自动选择：{_agent_display}",
                self.researcher.websocket
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"研究步骤已完成。\n💸 总研究费用：${self.researcher.get_costs()}",
                self.researcher.websocket,
            )
            if self.json_handler:
                ...
                if mcp_strategy == "disabled":
                    ...
                    if self.researcher.verbose:
                        await stream_output(
                            "logs",
                            "mcp_cache_reuse",
                            f"♻️ 为 {sub_query} 复用缓存的 MCP 结果（{len(mcp_context)} 个来源）",
                            self.researcher.websocket,
                        )
                    ...
                    if self.researcher.verbose:
                        await stream_output(
                            "logs",
                            "mcp_fallback",
                            f"🔌 MCP 缓存不可用，正在对 {sub_query} 运行 MCP 研究",
                            self.researcher.websocket,
                        )
            ...
            if combined_context:
                ...
                if self.researcher.verbose:
                    ...
                    await stream_output(
                        "logs",
                        "context_combined",
                        f"📚 已合并研究上下文：{mcp_count} 个 MCP 来源{cache_status}，{'有网页内容' if web_available else '无网页内容'}",
                        self.researcher.websocket,
                    )
            else:
                    ...
                    if is_mcp_retriever:
                        if self.researcher.verbose:
                            await stream_output(
                                "logs",
                                "mcp_results",
                                f"✓ 已从 MCP 服务器获取 {result_count} 条结果",
                                self.researcher.websocket,
                            )
