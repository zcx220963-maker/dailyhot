...
from ..memory import OPENAI_EMBEDDING_MODEL
...
class ContextCompressor:
    ...
    async def async_get_context(self, query: str, max_results: int = 5, cost_callback=None) -> str:
        ...
        Falls back to direct return when embeddings provider is unavailable (e.g. 404).
        ...
        try:
            ...
            # Embeddings provider unavailable (e.g. 404) or other error —
            # fall back to returning the raw content directly so research
            # can still proceed without embeddings.
            import logging
            logging.getLogger(__name__).warning(
                f"Embedding compression failed ({e}); falling back to direct context return"
            )

        # Fallback: return raw content from all documents (best effort)
        fallback_docs = [
            Document(
                page_content=doc.get('raw_content', ''),
                metadata=doc
        ...
        return self.prompt_family.pretty_print_docs(fallback_docs, len(fallback_docs))
...
class WrittenContentCompressor:
    ...
    async def async_get_context(self, query: str, max_results: int = 5, cost_callback=None) -> list[str]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(
                f"WrittenContentCompressor: embeddings failed ({e}); returning raw sections"
            )

        # Fallback: return top-N written sections without embedding filter
        return self.__pretty_docs_list([
            type('Doc', (), {'metadata': {'section_title': doc.get('section_title', f'Section {i+1}')}, 'page_content': doc.get('content', '')}, )
            for i, doc in enumerate(self.documents[:max_results])
        ], max_results)
